...
**Умная конвертация по словам:** буфер разбивается по пробелам, и каждое слово оценивается отдельно — частотами букв русского и английского. Слова, которые уже выглядят нормально в текущей раскладке (например, корректно набранное «меня»), не трогаются. Конвертируются только те, что в целевой раскладке выглядят осмысленнее, чем в текущей (например, «ЩЫШТЕ» → «OSINT»). Это позволяет в одном сообщении исправлять только реально неправильно набранные куски, не ломая остальное.
...
**Per-word smart conversion:** the buffer is split by spaces and each word is judged independently using letter-frequency tables for RU and EN. Words that already look fine in the current layout (e.g. a correctly typed «меня») are left alone. Only words that look more meaningful in the target layout (e.g. «ЩЫШТЕ» → «OSINT») get converted. This lets you fix only the truly wrong fragments in a message without breaking the rest.
