...
**Область переключения:** от последней `.` или `,` до курсора. Если точек/запятых нет — весь текст до начала сообщения. Границы ищутся по **целевой** раскладке — в RU раскладке KEY_DOT это буква «ю», а не точка, поэтому ложных разрывов не будет.
...
**Switch scope:** from the last `.` or `,` to the cursor. If none found — the entire text up to the start of the message. Boundaries are detected using the **target** layout — in the RU layout KEY_DOT is the letter «ю», not a period, so no false splits occur.
