...
**Область переключения:** от последней `.` или `,` до курсора. Если точек/запятых нет — весь текст до начала сообщения.
...
**Switch scope:** from the last `.` or `,` to the cursor. If none found — the entire text up to the start of the message.
